  ...
  <n-card>
    <div class="deploy">
      ...
      <n-steps :current="current" :status="currentStatus">
        <n-step
          title="第一步"
          description="左侧区域为已有host，您可以进行修改，之后进行提交，确认修改后，右侧区域会显示出改动对比（用颜色区分增删），此时您可以确认提交或撤回 ，成功提交后可进到下一步。
  "
        ></n-step>
        <n-step
          title="第二步"
          description="左侧区域为已有磁盘，您可以进行修改，之后进行提交，确认修改后，右侧区域会显示出改动对比（用颜色区分增删），此时您可以确认提交或撤回 ，成功提交后可进到下一步"
        ></n-step>
        <n-step
          title="第三步"
          description="左侧区域为磁盘格式化进度，您可以选择下一步直接跳过，或者选择“格式化”"
        ></n-step>
        <n-step
          title="第四步"
          description="获取当前集群配置（如果没有则为空），如果不存在，您可以新建集群，填写集群名称，集群配置并且进行集群创建。但集群名称不可修改
  "
        ></n-step>
        <n-step
          title="第五步"
          description="改步骤会显示集群的配置，用户确认无误后可进行部署"
        ></n-step>
      </n-steps>
      <div class="deploy-content">
        ...
        <div class="step_control_btn">
          <n-button-group>
            <n-button :disabled="current === 1" @click="pre">上一步</n-button>
            <n-button v-if="current !== 5" @click="next">下一步</n-button>
            <n-button v-if="current === 5" @click="submit">部署</n-button>
          </n-button-group>
        </div>
      </div>
    </div>
