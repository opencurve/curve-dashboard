<template>
    ...
    <n-card>
      <div class="header-bar">
        <div v-if="!handleStatus" class="batch-button">
          <n-button color="#2080f0" @click="batchHandle" size="small">批量操作</n-button>
        </div>
        <div v-else class="button-group">
          <n-button-group size="small" v-if="handleStatus">
            <n-button type="error" @click="showBatchDeleteVolumeModal" size="small">
              批量删除
            </n-button>
            <n-button type="default" @click="cancelBatch" size="small">
              取消
            </n-button>
          </n-button-group>
        </div>
        <div v-if="!router.currentRoute.value.params.path.includes('RecycleBin')" class="trash-bin">
          <n-button strong color="#255DBE" @click="goToTrashBin" size="small">
            <template #icon>
              <n-icon size="17">
        ...
        <div class="right-group">
          <div class="search-bar">
            <n-input placeholder="请输入卷名..." size="small" clearable v-model:value="pathToSearch"
          ...
          <div class="create-volume-namespace">
            <n-button v-if="!router.currentRoute.value.params.path.includes('RecycleBin')" size="small"
              @click="showCreateVolumeNamespace">+卷命名空间</n-button>
          </div>
          <div class="create-volume" v-if="!router.currentRoute.value.params.path.includes('RecycleBin')">
            <n-button color="#21d69d" size="small" @click="showCreateVolume({ volumeType: 1 })">+创建卷</n-button>
          </div>
        </div>
      </div>
      <div class="breadcrumb">
        <n-breadcrumb>
          <n-breadcrumb-item v-for="itm, index in pathArr" :clickable="index === pathArr.length - 1 ? true : false"
      ...
      <div class="data-form">
        <n-data-table :columns="columns" :data="volumeListData" :checked-row-keys="checkedRowKeys" :bordered="false"
          :row-key="rowKey" @update:checked-row-keys="handleCheck" @update:sorter="handleSorterChange" />
        <n-space justify="end">
  ...
  <CreateVolumeModal ref="createVolumeModalRef" @createVolume="onCreateVolumeNamespaceOrVolume" />
  <CreateVolumeNamespaceModal ref="createVolumeNamespaceModalRef" @createNamespace="onCreateVolumeNamespaceOrVolume" />
  <VolumeThrottleModal ref="volumeThrottleModalRef" />
  <VolumeExtedModal ref="volumeExtedModalRef" />
  <CreateSnapshotModal ref="createSnapshotModalRef" :rowInfo="rowInfo" />
  ...
  <BatchDeleteVolumeModal ref="batchDeleteVolumeModalRef" @confirm-batch-delete="batchDeleteVolume" />
</template>
  
<script lang="tsx" setup>
import { toRefs, ref, reactive, watch, h } from 'vue'
import { NDataTable, NIcon, NButton, NSpace, NPagination, NCard, NBreadcrumb, NBreadcrumbItem, NInput, useMessage, NModal } from 'naive-ui'
import type { DataTableColumns, DataTableColumn, DataTableSortState } from 'naive-ui'
...
const { getVolumeList, getVolumeDetail, deleteVolume, recoverVolume, lazyCloneVolume, createSnapshot, state } = blockStore;
let { volumeList } = toRefs(blockStore.state);
let routePath = ref(router.currentRoute.value.params.path);
let currentPath = ref(routePath.value === '' ? '/' : '/' + routePath.value.join('/') + '/');
...
const getPathArr = () => {
  ...
  if (arr.length === 1) { arr = ['home'] } else { arr.shift(); arr.unshift('home'); pathArr.value = [...arr] }
}
...
const deleteRow = (row: object) => {
  ...
  deleteVolume({ volumeNames: { [fileName]: fileType }, force: row.inRecycleBin }).then(res => {
    if (res.status === 200) {
...
const batchDeleteVolume = () => {
  deleteVolume({ volumeNames: checkedFileNames.value, force: window.location.pathname.includes('RecycleBin') ? true : false }).then(res => {
    if (res.status === 200) {
      if (window.location.pathname === '/recycleBin') {
        message.success('彻底删除卷成功');
      } else {
...
const cloneVolume = (row) => () => {
  ...
  const currentPath = ref(routePath.value === '' ? '/' : '/' + routePath.value.join('/') + '/');
...
const searchByPath = () => {
  ...
  // pathArr.value = ['home'];
  // getPathArr();
  let searchParams = { volumeName: pathToSearch.value }
  if (pathToSearch.value === "") {
    let _pathArr = router.currentRoute.value.params.path;
    _pathArr = '/' + (_pathArr.length === 0 ? '' : _pathArr.join('/'));
    getVolumeList({ page: 1, size: 10, path: _pathArr, sortKey: 'id', sortDirection: -1 })
      .then(res => {
        totalPage.value = Math.ceil(res.data.data.total / 10);
        page.value = 1; volumeListData.value = volumeList.value;
      })
  }
  else {
...
const operateColumn: DataTableColumn<getVolumeApiDataItem> = reactive({
  ...
  render: (row) => {
    if (router.currentRoute.value.params.path.includes('RecycleBin')) {
      return (
        <div>
          <n-button size="tiny" disabled={['kFileDeleting', 'kFileCloneMetaInstalled', 'kFileCloning'].includes(row.fileStatus)} type="normal" onClick={recoverVolumeFromBin(row)}>恢复</n-button>
          <n-button size="tiny" type="error" onClick={showDeleteModal(row, true)}>彻底删除</n-button>
        </div>
      )
    }
      ...
      return (
        <div>
          <n-button size="tiny" disabled type="normal">不可操作</n-button>
        </div>
      )
    }
    else return (
      <div>
        <n-button size="tiny" type="normal" disabled={row.fileType === "INODE_DIRECTORY"} onClick={showVolumeThrottleModal(row)}>设置限流</n-button>
        <n-button size="tiny" disabled={row.fileType === "INODE_DIRECTORY"} type="normal" onClick={showVolumeExtedModal(row)}>扩容</n-button>
        <n-button size="tiny" disabled={row.fileType === "INODE_DIRECTORY"} onClick={cloneVolume(row)} type="normal">克隆</n-button>
        <n-button disabled={row.fileStatus !== "kFileCloneMetaInstalled" || row.fileType === "INODE_DIRECTORY"} size="tiny" onClick={onLazyCloneVolume(row)} type="normal">安装数据</n-button>
        <n-button size="tiny" disabled={row.fileType === "INODE_DIRECTORY" || !['kFileCreated', 'kFileCloned'].includes(row.fileStatus)} type="normal" onClick={showCreateSnapshot(row)}>打快照</n-button>
        <n-button size="tiny" disabled={row.mountPoints.length !== 0} type="error" onClick={showDeleteModal(row, false)}>删除</n-button>
      </div>
    )
  }
})
...
const handleItemClick = (row: getVolumeApiDataItem) => () => {
  ...
  const name = row.fileType === 'INODE_DIRECTORY' ? 'pathVolumeManagement' : 'pathVolumeDetail'
...
const columns: DataTableColumns<getVolumeApiDataItem> = reactive([
  {
    ...
    render(row) {
      if (row.fileType === "INODE_DIRECTORY") {
        return (
          <NButton size='small' type='info' style={{ padding: '0 4px' }} quaternary onClick={handleItemClick(row)}>
            <span>{row.fileName.length < 15 ? row.fileName : row.fileName.substring(0, 15) + '...'}</span>
            <span >
              <n-icon size="18" color="#2080f0">
                <svg viewBox="0 0 16 16"><g fill="none"><path d="M7.207 4h-.032l-1.113-.89A.5.5 0 0 0 5.75 3H4a2 2 0 0 0-2 2v.5h3.557L7.207 4z" fill="currentColor"></path><path d="M8.693 4L6.086 6.37a.5.5 0 0 1-.336.13H2V11a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2H8.693z" fill="currentColor"></path></g></svg>
              </n-icon>
            </span>
          </NButton>
        )
      }
      return <NButton size='small' type='info' style={{ padding: '0 4px' }} quaternary onClick={handleItemClick(row)}>
        <span>{row.fileName.length < 15 ? row.fileName : row.fileName.substring(0, 15) + '...'}</span>
      </NButton>
  ...
  },
  {
    ...
    render(row) {
      return (
        <span>{statusMap[row.fileStatus]}</span>
      )
    }
  },
  cTimeColumn,
...
watch(router.currentRoute, (value) => {
  ...
  //  routePath.value = router.currentRoute.value.params.path;
  //  currentPath.value = routePath.value === '' ? '/' : '/' + routePath.value.join('/') + '/';
  if (value.name !== 'pathVolumeManagement') return
