  ...
  <n-modal v-model:show="val" preset="dialog" title="打快照">
    <n-form :rules="rules" :model="formData">
      ...
      <n-form-item path="snapshotName" label="快照名称">
        <n-input
          v-model:value="formData.snapshotName"
          placeholder="请输入快照名称..."
        ></n-input>
      </n-form-item>
    </n-form>
    <template #action>
      <div>
        <n-button type="primary" @Click="confirmCreateSnapshot">确定</n-button>
...
import { type FormItemRule, type FormRules, useMessage } from 'naive-ui'
