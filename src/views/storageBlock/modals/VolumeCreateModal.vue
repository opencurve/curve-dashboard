<template>
    <n-modal v-model:show="visible" preset="dialog" :title="volumeTitle">
        <n-form :model="formData" :rules="rules" label-placement="left" label-width="auto">
            <!-- 卷类型 -->
            <n-form-item label="卷类型" v-if="selectVisible">
                <n-select v-model:value="volumeType" :options="voluemOptions" @update:value="onChangeVolumeType" />
            </n-form-item>
            <!-- 卷名称 -->
            <n-form-item v-if="volumeType === 1" path="volumeName" label="卷名称">
                <n-input-group>
                    <n-input-group-label>当前路径:{{ currentPath }}</n-input-group-label>
                    <n-input v-model:value="formData.volumeName" placeholder="请输入卷名称...">
                    </n-input>
                </n-input-group>
            </n-form-item>
            <!-- 卷名称（克隆卷目的名） -->
            <n-form-item v-if="volumeType === 2" path="dest" label="卷名称">
                <n-input v-model:value="formData.dest" placeholder="请输入卷名称..."></n-input>
            </n-form-item>
            <!-- 用户名 -->
            ...
            <n-form-item v-if="volumeType === 2" path="src" label="来源">
                ...
                <n-input v-model:value="formData.src" placeholder="请输入卷来源..." v-model:disabled="srcDisabled"></n-input>
            </n-form-item>
            <!-- 卷长度 -->
            <n-form-item v-if="volumeType === 1" path="volumeLength" label="卷长度">
                <n-input v-model:value="formData.volumeLength" placeholder="卷长度在10-20x1024 GiB..."></n-input>
            </n-form-item>
            ...
            <n-form-item v-if="volumeType === 1" path="stripUnit" label="条带长度">
                ...
                <n-input v-model:value="formData.stripUnit" placeholder="请输入条带长度..."></n-input>
            </n-form-item>
            <n-form-item v-if="volumeType === 1" path="stripCount" label="条带数">
                ...
                <n-input v-model:value="formData.stripCount" placeholder="请输入条带数..."></n-input>
            </n-form-item>
            <n-form-item v-if="volumeType === 2" path="stripCount" label="是否Lazy">
                <n-radio :checked="checkedValue === 'isLazy'" value="isLazy" @change="handelChange">
                    是
                </n-radio>
                <n-radio :checked="checkedValue === 'notLazy'" value="notLazy" @change="handelChange">
                    否
                </n-radio>
            </n-form-item>
        </n-form>
        <template #action>
            <div>
                ...
                <n-button @Click="confirmCreateVolume" type="primary">创建</n-button>
            </div>
        </template>
    </n-modal>
</template>
...
import { useMessage, type FormRules, type FormItemRule,  NSelect, NRadio, NFormItem } from 'naive-ui'
...
const volumeLengthValidator = () => {
    if (typeof (formData.volumeLength === "number" && formData.volumeLength > 10 && formData.volumeLength < 20 * 1024)) {
        return true
    }
    else {
        return false
