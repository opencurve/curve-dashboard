...
<template>
  <div class="container">
    ...
    <div class="status" v-if="offlineCount === 0 && onlineCount !==0">健康</div>
    <div class="status" v-else >不健康</div>
...
const chunkServerStatus= ref(null);

const dashBoardStore = useDashboardStore()
const {  getChunkServerStatus} = dashBoardStore;
...
const setupCard = () => {
  ...
  getChunkServerStatus().then(res => { 
  chunkServerStatus.value = res.data.data; 
    onlineCount.value = chunkServerStatus.value.onlineNum;
    offlineCount.value = chunkServerStatus.value.totalNum - onlineCount.value;
    onlinePercentage.value = (onlineCount.value/chunkServerStatus.value.totalNum)*100;
    version.value = chunkServerStatus.value.versions[0].version
})
}
