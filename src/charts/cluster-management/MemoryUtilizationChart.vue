...
const drawChart = (() => {
  ...
  const memUtilizationArray: Array<number> = []

    let memUtilization = fullServerDetail.value.memUtilization;
    memUtilization.forEach(item => {
      let convertedTime = timestampToTime(item.timestamp)
      timeStamp.push(convertedTime);
      let itemValue = +item.value;
      let val = itemValue.toFixed(2);
      memUtilizationArray.push(val);
    })
