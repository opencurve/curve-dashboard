...
const { fullServerDetail } = toRefs(state);
  ...
  const drawChart = (() => {
    ...
    const timeStamp: Array<string> = [];
      ...
      let trafficTransmit = fullServerDetail.value.networkTraffic.transmit;
      //获取timestamp并将其存入相应数组
      let timeItem = Object.values(trafficTransmit); 
    ...
    // 获取键值
    const keys = []
    for(let key in trafficTransmit) {
      keys.push(key);
    }
    ...
    timeItem.forEach(item => {
        ...
        item.forEach(valueItem => {   
          let val = valueItem.value/(8*1024*1024);
          val = val.toFixed(2);      
            valueData.push(val)
        })
    ...
    //整理出图标里需要的数据数组
    const charTransmitData = []
    for(let i = 0; i<keys.length; i++) {
        charTransmitData.push(
            {
            name: keys[i],
            type: 'line',
            data: receiveData[i]
